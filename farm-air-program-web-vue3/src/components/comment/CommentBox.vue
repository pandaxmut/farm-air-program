...
<script setup lang="ts">
  ...
  import { insertArticleComments } from '@/api/articleComments';
  ...
  const handleSubmit = async () => {
    const newComment: ArticleComments = {
      articleId: props.id, // 需要从父组件传入实际值
  ...
  const sendComment = async function sendComments() {
    ...
    const data = await insertArticleComments(articleComments.value)
    if (data.code === 200) {
